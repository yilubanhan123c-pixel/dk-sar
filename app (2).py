...
def render_report(report: dict) -> str:
    ...
    lines.append(f"\n**设备**: {node.get('equipment','N/A')}  |  **参数**: {node.get('parameter','N/A')}  |  **偏差方向**: {node.get('deviation_direction','N/A')}")
    lines.append(f"\n**正常值**: {node.get('normal_value','N/A')}  |  **当前值**: {node.get('current_value','N/A')}")
    lines.append("\n---")
    ...
    lines.append(f"- **物理校验**: {'✅ 通过' if not issues else f'已修正 {len(issues)} 个问题'}")
    return "\n".join(lines)
...
def analyze_streaming(user_input: str):
    ...
    try:
        ...
        progress = log(f"✅ **节点1 完成** — {context['equipment']} / {context['parameter']} / {context['deviation_direction']}")
    ...
    while round_num < config.MAX_REFLECTION_ROUNDS:
        ...
        yield "分析中，请稍候...", progress, "", "", None
        try:
            report = RAGAgent().run(context=context, correction_guidance=correction_guidance, reflection_rounds=round_num)
        ...
        try:
            ...
            if ref_result["passed"]:
                progress = log("✅ **节点3 通过** — 物理校验无问题！")
                yield "分析中，请稍候...", progress, "", "", None
                break
            else:
                n = len(ref_result["issues"])
                progress = log(f"⚠️ **节点3 发现{n}个物理问题** — 返回节点2修正\n" +
                    "\n".join(f"  - {i['description'][:55]}..." for i in ref_result["issues"][:2]))
...
def create_ui():
    with gr.Blocks(title="DK-SAR 化工安全分析系统") as demo:
        ...
        <div style="text-align:center;padding:24px 0 8px 0;">
          ...
          <p style="color:#555;margin:0;">双知识增强自适应推理 · LangGraph 多智能体编排 · 自动化 HAZOP 分析</p>
          <p style="color:#999;font-size:12px;margin-top:4px;">
            三节点流水线：上下文解析 → RAG增强生成 → 物理反思校验（最多3轮修正）
          </p>
        </div>""")
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1, min_width=280):
                ...
                user_input = gr.Textbox(label="场景描述",
                    placeholder="例如：反应釜温度从80°C升至120°C...", lines=5)
                ...
                rating_slider = gr.Slider(minimum=1, maximum=5, step=1, value=4,
                                          label="评分（1-5星）")
                comment_box = gr.Textbox(label="反馈意见（可选）",
                                         placeholder="分析结果是否准确？有哪些改进建议？",
                                         lines=2)
        ...
        # 绑定事件
        analyze_btn.click(fn=analyze_streaming, inputs=[user_input],
            outputs=[report_output, progress_output, scores_output, json_output, export_btn])
        feedback_btn.click(fn=submit_feedback, inputs=[rating_slider, comment_box],
...
**评估**：PCC 物理覆盖率 | CCC 因果完整性 | RDI 建议详细度 | LCC 案例关联度""")
